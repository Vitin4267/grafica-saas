...
### 🔴 A1 — o motor DIGITAL exige `FormatoFolha` numa MATERIA_PRIMA, e nenhuma tela grava isso: todo orçamento DIGITAL morre em `MATERIAL_SEM_FOLHA`
...
### 🔴 A2 — a FLEXOGRAFIA escolhe a bobina só pelo `custoMaterial`, ignorando que a bobina decide a metragem e, com ela, toda a rodagem
...
### 🔴 A3 — CHAPA_RIGIDA: a geometria da chapa vem do PRODUTO e o preço vem de outro registro, e o motor escolhe de propósito o maior formato pagando um preço só
...
### 🔴 B1 — `MaquinaBordado.custoHoraMaq` é validado, gravado e prometido na tela, mas nunca chega ao motor
...
### 🔴 B2 — TEMPO_MAQUINA valida a quantidade e nunca a usa; a tabela de faixas recalcula justamente a quantidade
...
### 🔴 B3 — acabamento com preço de compra em branco custa R$0,00 e nada avisa — é o único custo do motor sem essa trava
...
### 🔴 C1 — EDITORIAL não valida faixa de gramatura; o Offset valida, e o campo é digitação livre sem teto
...
### 🟡 A4 — `numeroCliques` passou a ser "cliques por FOLHA" (achado N4) mas todo texto de tela continua dizendo "por peça", e não há teto
...
### 🟡 A5 — `paginasPorCaderno` aceita qualquer inteiro ≥ 1, inclusive ímpar: o motor passa a cobrar meia folha física por exemplar
...
### 🟡 B4 — o piso `custoMinimo` do job é comparado contra um total que inclui a peça em branco, então nunca protege o serviço
...
### 🟡 B5 — a base M2/METRO_LINEAR do acabamento significa uma coisa diferente em cada motor, pra exatamente a mesma peça
...
### 🟡 B6 — `horasEstimadas` é um número só por item, e todo acabamento por HORA cobra as mesmas horas
...
### 🟡 B7 — "Número de setups" tem dois textos de ajuda que se contradizem e é o único campo que decide o custo de tela
...
### 🟡 B8 — o nº de cores que paga o clichê de etiqueta é digitado duas vezes, em campos que ninguém compara
...
### 🟡 B9 — setup-por-peça e bordado classificam custo em categorias trocadas na previsão de custo do pedido
...
### 🟡 B10 — setup-por-peça é o único motor com substrato que não barra substrato zerado
...
### 🟡 C2 — o EDITORIAL descarta o aviso de "gramatura aproximada" que o Offset ganhou no achado N12 — justamente o sinal que denunciaria o C1
...
### 🟡 D1 — a margem agregada do orçamento soma itens medidos contra bases de custo diferentes (com e sem overhead)
...
### 🟡 D2 — `Cliente.margemPadraoOverride` não tem teto, e o erro só aparece ao orçar, apontando pro lugar errado
