...
// armazenamento, 4) alerta de prazo de entrega por e-mail (5/3/0 dias, ver
// src/lib/alerta-prazo-email.ts — canal separado do alerta_atraso sob
// demanda que já existia) e 5) expiração de importação de planilha
// abandonada (ver expirarImportacoesAbandonadas em src/lib/lifecycle-cron.ts).
// Junta tudo numa única rota (em vez de várias), já que todas são "melhor
// esforço, roda 1x/dia, sem efeito colateral uma na outra" — mesmo
// raciocínio de manter poucas rotas de cron (o plano grátis da Vercel tem
// limite de crons).
//
...
export async function GET(request: NextRequest) {
  ...
  // Promise.allSettled em vez de Promise.all: as cinco tarefas são
  ...
  return Response.json({
    ...
    importacoesExpiradas:
      importacoesExpiradas.status === "fulfilled"
        ? importacoesExpiradas.value
        : { erro: String(importacoesExpiradas.reason) },
  });
}
