...
import { NovaPrensaForm } from "./NovaPrensaForm";

export default async function PrensasPage() {
  const usuario = await exigirUsuarioAutenticado();
  await exigirAssinaturaAtiva(usuario);
  await exigirVerModulo(usuario, "CONFIGURACOES");
  const podeEditar = await podeEditarModulo(usuario, "CONFIGURACOES");

  const prensas = await prisma.prensa.findMany({
  ...
  return (
    <div className="flex flex-1 flex-col">
      ...
      <main className="mx-auto w-full max-w-3xl flex-1 px-6 py-10">
        ...
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Prensas</h1>
          <p className="mt-1 text-slate-500">
            Cada prensa offset tem seu próprio custo de máquina, chapas e rodagem.
            Produtos com modelo de cálculo Offset escolhem uma prensa específica no
            catálogo.
          </p>
        </div>
