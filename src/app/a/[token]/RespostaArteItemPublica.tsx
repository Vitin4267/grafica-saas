...
import { responderArteItemPublica } from "./actions";

// Achado F5 da auditoria de abrangência (Parte 7) — gêmeo de
// RespostaArtePublica.tsx (cabeçalho), mas decidindo sobre UMA ArteItem por
// vez (arteItemId), não sobre o pedido inteiro. Mesmo princípio de "nome
// declarado obrigatório" (disputa de arte custa caro).
export function RespostaArteItemPublica({
  token,
  arteItemId,
  nomeItem,
  nomeSugerido,
  ...
  const [state, formAction, isPending] = useActionState(responderArteItemPublica, null);
        ...
        <form action={formAction} className="flex flex-col gap-3">
          ...
          <label className="flex flex-col gap-1.5">
            <span className="text-sm font-medium text-slate-700 dark:text-slate-200">
              O que precisa mudar em {nomeItem}?
            </span>
            <textarea
              name="comentario"
              rows={4}
