...
describe("golden #6 — cartão digital Q=500: imposição por folha (achado N4), custo por clique + substrato POR FOLHA", () => {
  it("preço final bate com o cálculo manual (nUp/numeroFolhas -> custoCliques + custoSubstrato -> composição padrão)", () => {
    ...
    // de 0,10 -> 236,30 -> precoUnitario = 236,30/500 = 0,4726 exato em 4
    // casas (achado da auditoria de precificação de 2026-09-21 — antes
    // arredondava pra 2 casas aqui, 0,48, perdendo precisão real; ver
    // compor.ts) -> precoFinal = 0,4726 × 500 = 236,30 (bate exatamente com
    // o alvo comercial, sem markup artificial nenhum).
    ...
    expect(resultado.precoUnitario.toNumber()).toBeCloseTo(0.4726, 6);
    expect(resultado.precoFinal.toNumber()).toBeCloseTo(236.3, 6);
  });
});
...
describe("golden #7 — camiseta serigrafia Q=200/2 telas: setup fixo + variável, sem nesting", () => {
  it("preço final bate com o cálculo manual (calcularSetupPorPeca -> composição padrão)", () => {
    const contexto: ContextoPrecificacao = {
      ...
      // achado A2 (substrato) tem golden dedicado logo abaixo. Achado B10
      // (2026-09-13): 0 sozinho agora exige materialFornecidoPeloCliente,
      // senão CUSTO_INVALIDO — mesma trava de Digital/Bordado.
    ...
    // 1336,50/200 = 6,6825 exato em 4 casas (achado da auditoria de
    // precificação de 2026-09-21 — antes arredondava pra 2 casas aqui, 6,69,
    // perdendo precisão real; ver compor.ts) -> precoFinal = 6,6825 × 200 =
    // 1336,50 (bate exatamente com o alvo comercial, sem markup artificial).
    ...
    expect(resultado.precoUnitario.toNumber()).toBeCloseTo(6.6825, 6);
    expect(resultado.precoFinal.toNumber()).toBeCloseTo(1336.5, 6);
  });
  ...
  it("custoMinimo age como piso mesmo dentro do dispatcher completo", () => {
    const contexto: ContextoPrecificacao = {
      ...
      viraFolha: false,
      parametros: PARAMS,
      // Achado B10 (2026-09-13): idem golden anterior — flag necessária.
      setupPorPeca: { custoSubstratoPorPeca: 0, materialFornecidoPeloCliente: true },
  ...
  it("achado A2: custoSubstratoPorPeca (ItemGrafica.precoCompra) entra no custoBase — camiseta em branco não é mais R$0", () => {
    ...
    // 10×15 = 150 (era R$0 antes do fix A2). Achado B4 (2026-09-13): o piso
    // custoMinimo=150 compara só contra o SERVIÇO (80+35=115 < 150 -> piso
    // age: max(150,115)=150), e o substrato soma DEPOIS, sempre por fora ->
    // custoBase = 150 + 150 = 300 (não 265 — essa era a soma direta que
    // deixava o piso inerte sempre que o substrato empurrava o total pra
    // cima sozinho).
    expect(resultado.metricas.custoSubstrato as number).toBeCloseTo(150, 6);
    expect(resultado.detalhes.material.toNumber()).toBeCloseTo(300, 6);
  });
});
